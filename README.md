...
*   **🎞️ 沉浸式体验**:
    ...
    *   **瀑布流相册**: 智能排列的甜蜜瞬间。
    *   **真实缩略图链路**: 上传后自动生成缩略图，列表优先加载 `thumbUrl`，大图查看使用 `displayUrl`。
...
- `DATA_FILE` 和 `UPLOAD_DIR` 可在测试或临时环境中覆盖默认路径。
...
```json
{"success":true,"status":"ok","storageDriver":"json"}
```
...
4.  **图片策略**: 当前上传后会生成原图与缩略图；列表优先使用 `thumbUrl`，灯箱查看使用 `displayUrl`。
...
│   ├── routes/       # 路由配置
│   └── app.js        # 后端入口
├── test/             # Node built-in API smoke tests
├── data/             # 数据存储 (JSON/DB)
└── package.json
```
